...
| **Subgrid** | 부모 그리드 상속 | 일관된 정렬 | 정밀도 vs 미지원 브라우저 |
| **@container** | 독립적 컴포넌트 | 위치별 최적화 | 모듈화 vs 설정 오버헤드 |
...
- **장점**: JS 없이 툴팁/팝오버를 브라우저 네이티브로 완벽하게 배치.
- **단점**: 아직 Chromium 계열 위주 지원. 복잡한 엣지 케이스 설정이 까다로움.
...
> [!WARNING]
> **RCS Fallback**
> `oklch(from ...)` 문법은 구형 브라우저에서 무시된다. 배경이 사라지는 대참사를 막으려면 기본 컬러값을 먼저 선언해두는 센스가 필요하다.
...
- **단점**: 그룹 내 ID 선택자가 포함될 경우 전체 명시도가 폭발하여 오버라이드가 매우 힘들 수 있음.
