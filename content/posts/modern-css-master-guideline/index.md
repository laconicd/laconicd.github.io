...
- **Pros (장점)**: JS 없이 툴팁/팝오버를 브라우저 네이티브로 완벽하게 배치.
- **Cons (단점)**: 아직 Chromium 계열 위주 지원. 복잡한 엣지 케이스 설정이 까다로움.
...
- **Logical (`inline-size`, `margin-block`)**
  ...
  - **단점**: 기존 물리적 방향(`top/left`)에 익숙한 경우 초기 학습 비용 발생.

- **Physical (`width`, `margin-top`)**
  - **장점**: 직관적이며 전 세계적으로 가장 널리 쓰이는 표준.
...
- **OKLCH (The Winner)**
  - **장점**: 사람이 느끼는 밝기가 일정(L). Relative Color Syntax와 결합 시 매우 강력함.
...
### 🌗 다크모드와 콘트라스트 (APCA)
- **Lightness Shifting**: 배경은 어둡게(L 10%~20%), 텍스트는 충분히 밝게(L 90%+) 설정.
- **Chroma Reduction**: 눈의 피로도를 낮추기 위해 다크모드에서는 채도(C)를 미세하게 낮춤.
...
- **Pros (장점)**: 복잡한 JS 라이브러리 없이 앱 같은 부드러운 화면 전환이 가능.
- **Cons (단점)**: 다중 동시 전환 제어가 어렵고 세밀한 타이밍 커스텀이 까다로움.
...
- **단점**: 그룹 내 ID 선택자가 포함될 경우 전체 명시도가 폭발하여 오버라이드가 매우 힘들어짐.
...
7. **모션 & 접근성**: 애니메이션을 추가하고 포커스 등 접근성 요소를 점검한다.
