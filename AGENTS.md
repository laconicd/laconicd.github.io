...
- **Formatting:**
  ...
  deno fmt
  ```
  Formats all supported files (TS, JS, JSON, MD). Configuration: `lineWidth: 120` (in `deno.json`).
...
- **Testing:**
  ...
  deno test
  ```
  Runs tests if present (e.g., `*_test.ts` files). Currently, explicit tests may not be set up, but this is the standard
  command.
...
- **Lume Types**: Import types from `lume/core.ts` or specific plugins if needed (though mostly handled by `deno.json`
  types config).
