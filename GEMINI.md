...
This repository contains a static blog/journal site built with **Lume**, a fast and flexible static site generator for
**Deno**. The project emphasizes modern web aesthetics, featuring fluid typography, editorial layouts, and a clean
"lofi" theme.
...
- **Naming:** Follow existing patterns (snake_case for components like `post_card.vto`, kebab-case for some directory
  names).
...
- **Front Matter:** Required fields include `title`, `date`, `type: post`, and `layout`. Optional fields: `description`,
  `category`, `tags`, `img`.
